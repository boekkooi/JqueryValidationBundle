    ...
    public function provide_constraint_rule_expectation()
    {
        return array(
            array(
                ...
                new ConstraintRule('max', 0, new RuleMessage('msg', array('{{ compared_value }}' => 1)), array(Constraint::DEFAULT_GROUP)),
            ),
            array(
                new Constraints\LessThanOrEqual(array('value' => 1, 'message' => 'my_msg')),
                new ConstraintRule('max', 1, new RuleMessage('my_msg', array('{{ compared_value }}' => 1)), array(Constraint::DEFAULT_GROUP)),
            ),
            array(
                new Constraints\Range(array('max' => 3, 'maxMessage' => 'range_msg')),
                new ConstraintRule('max', 3, new RuleMessage('range_msg', array('{{ limit }}' => 3)), array(Constraint::DEFAULT_GROUP)),
            ),
        );
    }
